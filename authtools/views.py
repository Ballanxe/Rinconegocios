...
from .forms import AuthenticationForm, UserCreationForm, CaseInsensitiveUsernameFieldCreationForm
...
class VistaDePruebaParaDiseno(TemplateView):
    template_name= "registration/password_reset_confirm.html"
...
class RegisterUserView(FormView, RedirectView):
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(reverse('services:list'))
...
class LoginBigView(LoginView):
    ...
    #     if not allow_authenticated and self.request.user.is_authenticated():
    #         return redirect(self.get_success_url())
    #     return super(LoginBigView, self).dispatch(*args, **kwargs)

    # def get_form_class(self):
    #     return self.authentication_form or self.form_class

    def form_invalid(self, form):
        response = super(LoginBigView, self).form_invalid(form)
        # This method is called when valid form data has been POSTed.
        # It should return an HttpResponse.
        return response
    ...
    def form_valid(self, form):
        auth.login(self.request, form.get_user())
        super(LoginBigView, self).form_valid(form)
        
        return HttpResponseRedirect(reverse('services:list'))


    # def get_context_data(self, **kwargs):
    #     kwargs = super(LoginBigView, self).get_context_data(**kwargs)
    #     kwargs.update({
    #         self.redirect_field_name: self.request.GET.get(
    #             self.redirect_field_name, '',
    #         ),
